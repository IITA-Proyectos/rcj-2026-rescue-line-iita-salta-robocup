...
url = "/home/pi/Downloads/xdsdsd.mp4"  # o prueba /mjpeg si no funciona
# ---- cargar modelo ----
...
if ENABLE_WARMUP:
    ...
    model.predict(dummy, imgsz=IMGSZ, conf=0.25, iou=0.45, stream=False, verbose=False)
...
def infer_thread():
    ...
    while True:
        ...
        if frame_idx % DETECT_EVERY == 0:
            ...
            pred_ms = (time.perf_counter() - p0) * 1000.0
            pred_times_ms.append(pred_ms)
            if len(pred_times_ms) <= PRED_LOG_FIRST_N:
                print(f"PRED[{len(pred_times_ms)-1}] = {pred_ms:.1f} ms")
            detections = []
            for res in results:
