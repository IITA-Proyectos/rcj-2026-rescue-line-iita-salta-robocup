# -*- coding: utf-8 -*-
# ============================================================================
# main_rpi_2026-08-22.py - EL CODIGO DE VISION QUE REALMENTE CORRIO EL 22-AGO
# ============================================================================
#
# PROCEDENCIA: es el ~/Desktop/main.py de la Raspberry, tal como estaba el
# 2026-08-22, el dia en que se grabaron las 10 corridas de video y los 6 CSV de
# pista. Lo aporto Benjamin en la sesion de trabajo; se versiona aca porque
# hasta ahora NINGUNA conclusion de vision era reproducible: el codigo que
# generaba los datos no estaba en git.
#
# NO ES el archivo que se ejecuta en el robot. El robot corre su propio
# ~/Desktop/main.py. Este es la FOTO de ese archivo, para poder replicar el
# analisis. Si se cambia el de la Pi, hay que volver a fotografiarlo.
#
# NO CONFUNDIR con Main.py (mayuscula) de esta misma carpeta, que es una version
# distinta y mas vieja: tiene el recorte del ROI en la fila 55 y una rutina de
# linea perdida (last_line_search_dir) que ESTE archivo no tiene. En Windows los
# dos nombres colisionarian, y por eso este lleva fecha.
#
# COMO SE VERIFICO que es el que corrio: reimplementando su atan2 y comparandolo
# contra el rxsteer del CSV de la unica corrida que tiene video Y telemetria
# (hist.avi <-> 2026-08-22_pista_pivote_con_histeresis.csv):
#     con el recorte en la fila 60 (este archivo):  81,1 % de coincidencia
#     con el recorte en la fila 55 (Main.py):       58,1 %
#
# LO QUE HAY QUE MIRAR ACA, porque es donde esta el bug de linea perdida:
#     linea  60   min_line_size = 1
#     linea 766   black_mask[:60, :] = 0        <- el ROI es filas 60..119
#     linea 831   if np.sum(black_mask) < min_line_size:
#     linea 832       angle = 0
#
# black_mask es una mascara de 0/255, asi que "suma < 1" solo se cumple con CERO
# pixeles negros en todo el ROI. Medido sobre los 13.900 frames grabados: esa
# rama se dispara en el 5,32 % de los frames, pero la linea esta realmente
# perdida en el 20,9 %. El robot se entera de 1 de cada 4 perdidas; en las otras
# tres calcula un angulo confiado sobre el zocalo y las patas de las mesas.
# ============================================================================
