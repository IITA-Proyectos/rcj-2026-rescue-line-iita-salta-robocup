...
def modo_rescate():
    ...
    STOP_WIDTH_RATIO_BOX_EVAC = 0.21
            ...
            try:
                if ser.in_waiting > 0:
                    ...
                    if data == b'\xff':
                        ...
                        estado = 'depositar'
                    elif data == b'\xf7' and estado in ("rescate", "depositar", "depositar verde"):
                        print("Llego 247 -> cambiar a evacuacion (seguir en modo_rescate)")
                        estado = 'evacuacion'
            except Exception as e:
    ...
    def main_loop():
        ...
        while True:
            ...
            if target:
                ...
                elif estado == "evacuacion":
                    close_enough = width_ratio >= STOP_WIDTH_RATIO_BOX_EVAC
                else:
                ...
                if close_enough:
                    ...
                    elif ball_type == "green_zone": green_state = 9

                    if estado != "evacuacion" and ball_type == "green_zone":
            ...
            if estado == "evacuacion":
                ...
                frame_small = cv2.resize(frame, (160, 120), interpolation=cv2.INTER_NEAREST)
                roi = frame_small[95:, :, :]
                if ENABLE_ANTIFLASH:
                    roi = anti_flash_preprocess(roi)
                roi_agcwd       = agcwd(roi)
                black_mask_evac = cv2.inRange(roi_agcwd, lower_black, upper_black)
                black_pixels    = np.sum(black_mask_evac) // 255
                # Si hay triangulo (8/9), no sobreescribir con estado de salida
                if green_state not in (8, 9):
                    if black_pixels > 300:
                        confirmaciones_negro += 1
                        if confirmaciones_negro >= 5:
                            green_state = 20  # salida confirmada
                            confirmaciones_negro = 0
                        else:
                            green_state = 0   # aun no confirmada
                    else:
                        confirmaciones_negro = 0
                        green_state = 0       # seguir buscando

                # Teensy va derecho, no usamos angulo/velocidad
                speed = 0
                angle = 90
                print("EVAC green_state:", green_state, "w_ratio:", f"{width_ratio:.3f}" if target else "None")
