...
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode) {
  ...
  delay(30);
  // ACOTADO POR IITA (2026-08-22). El original era:
  //     while (read8(BNO055_CHIP_ID_ADDR) != BNO055_ID) { delay(10); }
  // Sin salida. begin() manda un reset por software al BNO y despues espera
  // PARA SIEMPRE a que vuelva a contestar. Si el sensor no vuelve -pasa, y es
  // intermitente- begin() no retorna nunca y setup() queda colgado: LED apagado
  // y puerto mudo, en la pista y delante del arbitro. El manejo de "el BNO no
  // responde" del firmware solo corre si begin() DEVUELVE, asi que no ayudaba.
  // Con el techo, si el sensor no vuelve en 2 s begin() devuelve false y el
  // robot arranca sin IMU en vez de no arrancar. Perder la IMU cuesta el
  // veredicto por giro; no arrancar cuesta la corrida.
  {
    uint32_t _t0 = millis();
    while (read8(BNO055_CHIP_ID_ADDR) != BNO055_ID) {
      if (millis() - _t0 > 2000) return false;
      delay(10);
    }
  }
  delay(50);
