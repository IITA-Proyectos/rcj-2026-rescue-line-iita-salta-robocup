...
app.controller('ddController', ['$scope', '$uibModal', '$log', '$timeout', '$http', '$translate', '$cookies', function ($scope, $uibModal, $log, $timeout, $http, $translate, $cookies) {
  ...
  function upload_run(data) {
    ...
    $http.put("/api/runs/line/" + runId, Object.assign(data, {
      ...
      // [rcj-line-offline] flag corregirBugsVisuales: $http expone .status (no .statusCode) y $scope.upload_run no existe
      if (RCJLocal.flags.corregirBugsVisuales && response.status == 202) {
        $scope.sync--;
        $timeout(function () { upload_run(data); }, 100);
        return;
      }
      $scope.sync--;
  ...
  if (document.referrer.indexOf('sign') != -1 || rcjVolviDeFirma(runId)) { // [rcj-line-offline] + marca local de firma.html (juez.html A1)
    $scope.checked = true;
    $scope.fromSign = true;
  ...
  $scope.calc_victim_multipliers = function (index){
    ...
    if (victim.victimType == "DEAD") {
      ...
      for (i of $scope.range(index)) {
        ...
        if (v.victimType == "LIVE" && (!RCJLocal.flags.corregirBugsVisuales || v.zoneType == "GREEN")) liveCount ++; // [rcj-line-offline] flag: solo vivas efectivas, como el servidor (2026.js:59-69)
      }
  ...
  $scope.saveEverything = function () {
    ...
    $http.put("/api/runs/line/" + runId, run, http_config).then(function (response) {
      ...
      // [rcj-line-offline] flag corregirBugsVisuales: $http expone .status (no .statusCode)
      if (RCJLocal.flags.corregirBugsVisuales && response.status == 202) {
        $scope.sync--;
        $timeout($scope.saveEverything, 100);
        return;
      }
      $scope.score = response.data.score;
  ...
  $scope.go = function (path) {
    playSound(sClick);
    window.location = RCJLocal.ruta(path) // [rcj-line-offline] regla 3b: rutas del CMS -> páginas locales
  };
