...
var txt_bump;
var rotate = 0;
// [rcj-line-offline] desvíos por flag de RCJLocal.flags (ESPEC D4 corregirBugsVisuales, D6 soloFirmaCapitan).
// Con los flags en su valor por defecto (false) todo se comporta como el CMS. Ver cambios/firma.md.
function rcjFlag(nombre) {
    try {
        return !!(window.RCJLocal && window.RCJLocal.flags && window.RCJLocal.flags[nombre]);
    } catch (e) {
        return false;
    }
}
...
app.controller('ddController', ['$scope', '$uibModal', '$log', '$timeout', '$http', '$translate', '$cookies', function ($scope, $uibModal, $log, $timeout, $http, $translate, $cookies) {
    ...
    // [rcj-line-offline] D4: la plantilla usa este valor para la numeración de la tabla de rescate (j*4 -> j*3)
    $scope.rcjCorregirBugsVisuales = rcjFlag('corregirBugsVisuales');
        ...
        if (typeof runId !== 'undefined') {
            ...
            socket.on('data', function (data) {
                ...
                let tmp = {
                    ...
                    // [rcj-line-offline] D4 corregirBugsVisuales: el servidor suma +5 si showedUp || puntos
                    // (scoreCalculatorRules/2026.js:81-83); con showedUp=false y puntos, raw_score > 0.
                    point: rcjFlag('corregirBugsVisuales') ? 5 * ($scope.showedUp || $scope.raw_score > 0) : 5 * $scope.showedUp
                }
      ...
      $scope.calc_victim_multipliers = function (index){
        ...
        if (victim.victimType == "DEAD") {
          ...
          for (i of $scope.range(index)) {
            ...
            // [rcj-line-offline] D4 corregirBugsVisuales: contar solo las LIVE efectivas (no en RED), como el
            // servidor (scoreCalculatorRules/2026.js:61-68). Sin el flag cuenta todas, como el CMS.
            if (v.victimType == "LIVE" && !(rcjFlag('corregirBugsVisuales') && v.zoneType == "RED")) liveCount ++;
          }
    ...
    $scope.go = function (path) {
        ...
        // [rcj-line-offline] regla 3b: las rutas del CMS pasan por RCJLocal.ruta (/line/judge/<id> -> juez.html?run=<id>).
        // "To correct" (firma -> juez): el juez del CMS saltea el pre-chequeo si document.referrer contiene "sign"
        // (judge/line_2026.js:202); acá la firma es firma.html, así que se deja la marca de sesión
        // sessionStorage['rcjVolviDeFirma'] = runId para que el área juez la lea. Ver cambios/firma.md.
        if (/^\/line\/judge\//.test(path)) {
            try { sessionStorage.setItem('rcjVolviDeFirma', runId); } catch (e) { /* sin sessionStorage */ }
        }
        window.location = RCJLocal.ruta(path);
    }
    ...
    $scope.send_sign = function () {
        ...
        // [rcj-line-offline] D6 soloFirmaCapitan: el reglamento 2026 solo exige la del capitán; con el flag las del
        // árbitro y el árbitro asistente son opcionales (si están se envían igual). Sin el flag, 3 firmas como el CMS.
        if (!$scope.signData[1]) {
            if (!rcjFlag('soloFirmaCapitan')) err_mes += "[" + txt_ref_sign + "] "
        } else {
            run.sign.referee = $scope.signData[1]
        }

        if (!$scope.signData[2]) {
            if (!rcjFlag('soloFirmaCapitan')) err_mes += "[" + txt_cref_sign + "] "
        } else {
...
app.controller('ModalInstanceCtrl', ['$scope', '$uibModalInstance', 'mtile', 'mtiles', 'stiles', 'nineTile', 'sRotate', 'startTile', 'startTile2', 'isCheckPoint', function ($scope, $uibModalInstance, mtile, mtiles, stiles, nineTile, sRotate, startTile, startTile2, isCheckPoint) {
    ...
    // [rcj-line-offline] D4 corregirBugsVisuales: templates/line_view_modal.html cambia el ícono FA4 fa-play-circle-o
    // (sin glifo en FA5) por far fa-play-circle
    $scope.rcjCorregirBugsVisuales = rcjFlag('corregirBugsVisuales');
    $scope.evacTapeRot = function (tile) {
        let rot = 0;
        if (tile.evacEntrance >= 0) {
