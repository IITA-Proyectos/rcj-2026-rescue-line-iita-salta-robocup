...
app.controller('RunAdminController', ['$scope', '$http', '$log', '$location', 'Upload', function ($scope, $http, $log, $location, Upload) {
        ...
        $http.get(`/api/competitions/${competitionId}`).then(function (response) {
            ...
            $scope.ranking = {
                comment: `${$scope.league.name} - Round Scores`
            };
        })
                    ...
                    activeGroups.forEach(group => {
                        ...
                        if (g && g.normalizedScore !== null) {
                            ...
                            if (g.timeSeconds != null) {
                                ...
                                if (bestTime === null || g.timeSeconds < bestTime) {
                                    bestTime = g.timeSeconds;
                                }
                            }
                        }
                    });
        ...
        $scope.go = function (path) {
            ...
            window.location = RCJLocal.ruta(path) // [rcj-line-offline] regla 3b: navegación por RCJLocal.ruta
        }
