...
app.controller('LineEditorController', ['$scope', '$rootScope', '$uibModal', '$log', '$http', '$translate', function ($scope, $rootScope, $uibModal, $log, $http, $translate) {
    ...
    $scope.go = function (path) {
        window.location = RCJLocal.ruta(path) // [rcj-line-offline] regla 3b
    }
    ...
    $scope.saveMapAs = function (newName, targetCompetitionId) {
        ...
        $http.post("/api/maps/line", map).then(function (response) {
            ...
            window.location.replace(RCJLocal.ruta("/admin/" + targetCompetitionId + "/" + leagueId + "/mapEditor/" + response.data.id)) // [rcj-line-offline] regla 3b
        }, function (response) {
            console.log(response);
            console.log("Error: " + response.statusText);
            Toast.fire({
    ...
    $scope.saveMap = function (callback = null) {
        ...
        if (mapId) {
            ...
            $http.post("/api/maps/line", map).then(function (response) {
                ...
                window.location.replace(RCJLocal.ruta("/admin/" + competitionId + "/" + leagueId + "/mapEditor/" + response.data.id)) // [rcj-line-offline] regla 3b
            }, function (response) {
                console.log("Error: " + response.statusText);
                if (callback == null) {
                    Toast.fire({
    ...
    // [rcj-line-offline] desvío D1 (ESPEC §2; 08 §5.2 "Adaptador permitido"): el import oficial espera `tiles`
    // como objeto "x,y,z" con `tileType` embebido. Si llega el formato del servidor (`tiles` array y/o
    // `tileType` como id) se convierte antes de asignar. Con el formato oficial no toca nada.
    function adaptarImportacionLocal(data, tileSets) {
        if (!data || typeof data !== 'object' || !data.tiles || typeof data.tiles !== 'object') return data;
        if (Array.isArray(data.tiles)) {
            var dic = {};
            for (var i = 0; i < data.tiles.length; i++) {
                var t = data.tiles[i];
                if (t && typeof t === 'object') dic[t.x + ',' + t.y + ',' + t.z] = t;
            }
            data.tiles = dic;
        }
        var sets = (tileSets || []).filter(function (s) { return s._id == data.tileSet; })
            .concat((tileSets || []).filter(function (s) { return s._id != data.tileSet; }));
        for (var k in data.tiles) {
            var tile = data.tiles[k];
            if (!tile || tile.tileType == null || typeof tile.tileType === 'object') continue;
            for (var j = 0; j < sets.length; j++) {
                var e = (sets[j].tiles || []).find(function (x) { return x.tileType && x.tileType._id == tile.tileType; });
                if (e) { tile.tileType = e.tileType; break; }
            }
        }
        return data;
    }
    ...
    if (window.File) {
        ...
        select.addEventListener('change', function (e) {
            ...
            reader.onload = function () {
                ...
                data = adaptarImportacionLocal(data, $scope.tileSets); // [rcj-line-offline] desvío D1
