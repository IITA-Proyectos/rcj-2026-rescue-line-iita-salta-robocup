...
### A. False Balls and Glare (Blinding Light)
*   **The Problem:** Sunlight or LED lights can create silver reflections on the ground or white walls. If the RPi is blinded and emits a false positive (`green_state = 6`), the Teensy will stop the robot and execute a collection "in the air."
...
| **328-369** | `runDistance()` | `while (fr.pulseCount <= encoder)`. If a wheel gets mechanically stuck (e.g. hair tangled in the axle), the encoder does not count pulses. The robot is left in an infinite loop. A watchdog timer is missing. |
...
| **604-608** | Logic `ball_counter>= 3` | Inflexible. If the robot failed a capture attempt, the counter still increased. The robot will think it has a full belly and will go to deposit nonexistent balls too early. |
