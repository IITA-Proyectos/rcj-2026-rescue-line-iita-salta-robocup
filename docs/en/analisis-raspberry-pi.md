...
2.  **Line Following:** 
    *   Uses the **Weighted Center of Mass** method. Black pixels at the bottom of the frame carry more weight in the error calculation than those at the horizon.
...
1.  **Deferred Model Loading:** Move the initialization of YOLO to the start of the main script (`Main.py`) so that it only occurs once.
2.  **Integration of Distance Sensors:** Currently, the RPi "estimates" the distance to the ball by its size in pixels. It is recommended to use readings from the **ToF (VL53L0X)** sensors on the Teensy to validate that the ball is actually in front of the claw before closing.
